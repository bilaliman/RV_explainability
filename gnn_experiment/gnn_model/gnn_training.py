...
class GCN(torch.nn.Module):
    ...
    def forward(self,x, edge_index,batch):
        ...
        # 3. Concatenate with stance + Readout layer
...
for fold in ['ferguson','sydneysiege','germanwings-crash','ottawashooting','charliehebdo']:
    ...
    if not os.path.isfile(os.path.join('best_results_stance',fold+'_saved_gnn')):
        ...
        for epoch in range(1,301):
            ...
            for data in train_loader:  # Iterate in batches over the training dataset.
                ...
                correct_train += int((pred == data.y).sum())  # Check against ground-truth labels.
            ...
            train_acc = correct_train/len(train_list)
            if epoch%10==0:
                print('Epoch: {}: train accuracy {} and train loss {} and test loss {}'.format(epoch,train_acc,acc_loss,valid_loss))
            

            
            # Save state dictionary    
            model_path = os.path.join(fold+'_saved_gnn')
